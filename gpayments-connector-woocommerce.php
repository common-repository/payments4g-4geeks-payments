...
class WC_GPayments_Connection extends WC_Payment_Gateway
{
	...
	public function woogatewaypro_payment_ok()
	{
		...
		if ($JsonResponse->status == 'succeeded') {
			// Payment successful
			$customer_order->add_order_note(__('Pago completado.', 'wc-4gpayments'));
			wc_add_notice("Pago completado.", 'success');
			...
			if ($JsonResponse->status == 'canceled') {
				$customer_order->add_order_note('Error: El pago no pudo completarse (Cancelado).');
				wc_add_notice("El pago no fue completado o no tuvo autorización bancaria. Vuelva a intentarlo mas tarde o use una tarjeta distinta.", 'error');
			}

			if ($JsonResponse->status == 'requires_action') {
				$customer_order->add_order_note('El pago requiere acción por parte del comprador.');
				wc_add_notice("El pago no fue completado o no tuvo autorización bancaria. Vuelva a intentarlo mas tarde o use una tarjeta distinta.", 'error');
			}

			else {
				$customer_order->add_order_note('Error: El pago no tuvo autorización.');
				wc_add_notice("El pago no fue completado o no tuvo autorización bancaria. Vuelva a intentarlo mas tarde o use una tarjeta distinta.", 'error');
			}
